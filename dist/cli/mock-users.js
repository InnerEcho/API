...
const DEFAULT_PERSONAS = Object.keys(PERSONAS);
...
function parseArgs() {
  ...
  const personasRaw = (map.get('personas') ?? DEFAULT_PERSONAS.join(',')).split(',').map(s => s.trim().toUpperCase()).filter(Boolean);
  const personas = personasRaw.filter(isPersonaKey);
  if (!personas.length) personas.push(...DEFAULT_PERSONAS);
  const strToBool = (value, fallback) => {
...
function normalLike(rng, mean, sd = 0.7) {
...
async function ensureMockUsers(prefix, count, personas, rng, perPersona) {
  const created = [];
  const assignments = [];
  ...
  for (const persona of assignments) {
    ...
    await db.sequelize.query(`INSERT INTO \`user\` (user_email, user_name, password, user_gender, created_at)
    ...
    const userRows = await db.sequelize.query(`SELECT user_id FROM \`user\` WHERE user_email = :email LIMIT 1`, {
      replacements: {
        email
      },
...
async function loadMissionPool() {
  const rows = await db.sequelize.query(`SELECT mission_id, code, type, burden
     FROM missions
     WHERE is_active = 1`, {
    type: QueryTypes.SELECT
  });
...
function sampleMission(rng, pool, persona, lastCode) {
  const types = ['instant', 'habit', 'action', 'ar_optional'];
  ...
  candidates = candidates.sort((a, b) => Math.abs(a.burden - desiredBurden) - Math.abs(b.burden - desiredBurden));
  if (persona.noveltyPenchant > 0.5 && lastCode && candidates.length > 1 && rng() < persona.noveltyPenchant) {
...
async function loadExistingMockUsers(prefix, limit, personas, rng) {
  ...
  return rows.map(row => {
    ...
    const persona = isPersonaKey(key) ? key : fallback();
    return {
  ...
  for (const user of users) {
    ...
    for (let day = opts.days - 1; day >= 0; day--) {
      ...
      for (let k = 0; k < dailyCount; k++) {
        ...
        await db.sequelize.query(`INSERT INTO user_missions (
        ...
        if (outcomeRoll < persona.completionProb) {
          ...
          await db.sequelize.query(`UPDATE user_missions
             SET status = 'complete',
