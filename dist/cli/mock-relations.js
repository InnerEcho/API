...
function kstDate(daysAgo, hour, minute = 0) {
  const now = new Date();
...
function hourFromBucket(rng, bucket) {
 ...
 */
async function loadMockUsers(prefix) {
  const likePattern = `${prefix}+%`;
  const rows = await db.sequelize.query(`SELECT user_id, user_email FROM \`user\`
  ...
  return rows.map(row => {
    ...
    const persona = isPersonaKey(key) ? key : 'A1';
    return {
 ...
 * 2단계: 모든 모의 사용자에게 기본 식물(plant)을 할당합니다.
 */
async function ensureUserPlants(users) {
  ...
  for (const user of users) {
    ...
    await db.sequelize.query(`INSERT INTO \`plant\` (user_id, species_id, nickname, plant_level, plant_experience, plant_hogamdo, last_measured_date)
       VALUES (:userId, :speciesId, :nickname, 1, 0, 50, NOW())
       ON DUPLICATE KEY UPDATE
         nickname = VALUES(nickname),
         last_measured_date = NOW()`,
    // ON DUPLICATE KEY UPDATE에도 last_measured_date 추가
    {
      replacements: {
        userId: user.userId,
    ...
    const [plantRow] = await db.sequelize.query(`SELECT plant_id FROM \`plant\` WHERE user_id = :userId LIMIT 1`, {
      replacements: {
        userId: user.userId
      },
      type: QueryTypes.SELECT
...
const getPersonaEmotion = persona => {
...
const getMockMessages = persona => {
  const {
    emotion,
    factor
  } = getPersonaEmotion(persona);
  return {
 ...
 */
async function generateChatAndAnalysis(users, opts, rng) {
  const personaMap = PERSONAS;
  let chatCount = 0;
  let analysisCount = 0;
  for (const user of users) {
    ...
    for (let day = opts.days - 1; day >= 0; day--) {
      ...
      for (let i = 0; i < chatSessions; i++) {
        ...
        const chosenBucket = pickWeighted(rng, buckets, buckets.map(bucket => persona.timeBucketWeights[bucket]));
        const hour = hourFromBucket(rng, chosenBucket);
        const chatTime = kstDate(day, hour, Math.floor(rng() * 50));
        const {
          userMsg,
          botMsg
        } = getMockMessages(persona);

        // 1. 사용자 메시지 삽입 (plant_history)
        // [경고] 이 테이블이 DB에 없으면 여기서 실패합니다.
        const [userChatId] = await db.sequelize.query(`INSERT INTO \`plant_history\` (user_id, plant_id, message, user_type, send_date, created_at, updated_at)
        ...
        await db.sequelize.query(`INSERT INTO \`chat_analysis\` (history_id, emotion, factor, created_at)
        ...
        await db.sequelize.query(`INSERT INTO \`plant_history\` (user_id, plant_id, message, user_type, send_date, created_at, updated_at)
 ...
 */
async function generateDiaries(users, opts, rng) {
  let diaryCount = 0;
  for (const user of users) {
    if (!user.plantId) continue;

    // [수정됨] 'send_date' -> 'created_at' (chat_analysis 스키마 기준)
    const analyses = await db.sequelize.query(`SELECT
    ...
    for (const dateStr of uniqueDates) {
      ...
      const [missionRow] = await db.sequelize.query(`SELECT m.title
      ...
      await db.sequelize.query(`INSERT INTO \`growth_diary\` (user_id, plant_id, title, content, emotion, emotion_factor, primary_mission, date, created_at, updated_at)
 ...
 */
async function generateFriends(users, rng) {
  let friendCount = 0;
  ...
  for (const user of users) {
    ...
    for (let i = 0; i < targetFriendCount; i++) {
      ...
      await db.sequelize.query(`INSERT INTO \`user_friends\` (user_id, friend_id, status, created_at, updated_at)
