...
export class RealtimeSpeechServiceOld {
   ...
   */
  async loadChatHistory(userId, plantId) {
...
`.trim();
   ...
   */
  async createRealtimeConnection(userId, plantId, clientWs) {
    console.log(`🔌 Realtime API 연결 시작: userId=${userId}, plantId=${plantId}`);
    ...
    console.log(`💬 대화 히스토리 로드 완료: ${conversationHistory.length}개 메시지`);
    ...
    openaiWs.on('open', () => {
      ...
      console.log(`⚠️ 대화 히스토리 로드 비활성화됨 (${conversationHistory.length}개 메시지)`);
    });
    openaiWs.on('message', data => {
      const event = JSON.parse(data.toString());
      console.log(`📥 OpenAI 이벤트: ${event.type}`);
      if (event.type === 'error') {
        console.error('❌ OpenAI API 에러:', JSON.stringify(event.error, null, 2));
      }
    ...
    clientWs.on('message', message => {
      console.log(`➡️ 클라이언트로부터 메시지 수신: 크기=${message.length} bytes`);
   ...
   * 대화 히스토리 저장 처리
   */
  async handleHistorySave(event, userId, plantId) {
    try {
      const messageHistory = new RedisChatMessageHistory(userId, plantId);
      ...
      if (event.type === 'response.done') {
        ...
        if (output && output.length > 0) {
          const assistantMessage = output.filter(item => item.type === 'message').map(item => {
