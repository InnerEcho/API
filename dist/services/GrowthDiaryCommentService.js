...
export class GrowthDiaryCommentService {
    ...
    async updateComment(content, user_id, diary_id, comment_id) {
        ...
        try {
            const date = new Date();
            const updatedComment = await db.GrowthDiaryComment.update({ content: content, updated_at: date, edited: true }, {
                where: {
                    user_id,
                    diary_id,
                    comment_id,
    ...
    async deleteComment(user_id, diary_id, comment_id) {
        ...
        try {
            const date = new Date();
            const result = await db.GrowthDiaryComment.update({ is_deleted: true, updated_at: date }, {
                where: {
                    user_id,
                    diary_id,
                    comment_id,
...
//# sourceMappingURL=GrowthDiaryCommentService.js.map