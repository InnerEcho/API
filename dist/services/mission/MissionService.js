...
import { PlantStateService } from "../plant/PlantStateService.js";
import { loadRecentHistoryMap, noveltyPenaltyById } from "./recentHistory.js";
...
// ----------------------------------
function matchesEmotionTags(ctx, tags) {
  const pool = [];
...
function matchesCondition(mission, condition) {
  const {
    codes,
    codeIncludes,
    types
...
function contextScore(mission, ctx) {
  ...
  if (typeof ctx.arousal === 'number' && ctx.arousal <= lowArousal.threshold && typeof mission.burden === 'number' && mission.burden >= lowArousal.minBurden) {
...
export class MissionService {
  ...
  static async recommendIfEmpty(userId, n = 2) {
    ...
    if (maxBurden !== null && maxBurden !== undefined) {
      candidates = await db.sequelize.query(`SELECT * FROM missions WHERE is_active = 1 AND burden <= :maxBurden`, {
        replacements: {
          maxBurden
        },
        type: QueryTypes.SELECT
      ...
      candidates = await db.sequelize.query(`SELECT * FROM missions WHERE is_active = 1`, {
        type: QueryTypes.SELECT
      });
    }
    try {
