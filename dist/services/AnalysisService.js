...
export class AnalysisService {
  ...
  async extractFactor(message) {
    ...
    try {
      const response = await axios.post(this.factorEndpoint, {
        text: message
      });
      const {
        factor
  ...
  async analyzeAndStore({
    ...
    const [emotion, factor] = await Promise.all([this.analyzeEmotion(message), this.extractFactor(message)]);
    try {
