...
import { buildFullHistoryCacheKey, buildTodayHistoryCacheKey } from "./historyCache.util.js";
...
export class ChatHistoryService {
   ...
   * - 캐시 미사용
   */
  async fetchHistoryFromDb(userId, plantId) {
    ...
    return chatHistoryDb.map(item => this.convertDbToMessage(item));
  }
   ...
   */
  async getChatHistoryFromDb(userId, plantId) {
  ...
  async getTodayHistory(userId, plantId) {
    const {
      ...
      startUtc,
      endUtc
    } = this.buildTodayRange(userId, plantId);
  ...
  buildTodayRange(userId, plantId) {
    const nowKst = dayjs().tz('Asia/Seoul');
    const startUtc = nowKst.startOf('day').utc().toDate();
    const endUtc = nowKst.endOf('day').utc().toDate();
    const cacheKey = buildTodayHistoryCacheKey(userId, plantId, nowKst.format('YYYY-MM-DD'));
    return {
      cacheKey,
      startUtc,
      endUtc
