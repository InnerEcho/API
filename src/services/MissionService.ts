...
import { PlantStateService } from './PlantStateService.js'; // PlantStateService 임포트
...
export class MissionService {
  ...
  public async completeMission(
    ...
    const userEvent = await db.User_Event.findOne({
      where: { user_id: user_id, event_id: event_id, status: "assigned" },
      raw: true,
    });
    ...
    const eventInfo = await db.Event.findOne({ where: { event_id: event_id }, raw: true, });
    ...
    const plant = await db.Plant.findOne({ where: { user_id: user_id } });
    if (!plant) {
