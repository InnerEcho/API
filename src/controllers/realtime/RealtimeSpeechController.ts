...
export class RealtimeSpeechController {
  ...
  public async saveChatHistory(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.user?.userId;
      const { plantId, userMessage, assistantMessage } = req.body;
      ...
      await this.realtimeSpeechService.saveChatHistory(
        userId,
        plantId,
        userMessage,
        assistantMessage,
  ...
  public async getChatHistory(req: Request, res: Response): Promise<void> {
    try {
      ...
      const history = await this.realtimeSpeechService.getChatHistory(
        userId,
        plantId,
      );
