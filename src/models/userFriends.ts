...
interface UserFriendsAttributes {
  friend_id: number; // PK
  user_email: string; // 요청을 보낸 사용자 이메일 (FK)
  friend_email: string; // 친구 요청을 받은 사용자 이메일 (FK)
  friend_nickname: string; // UI에 보여질 별명
  status: "pending" | "accepted" | "rejected"; // 친구 요청 상태
}

// 모델 반환 타입 정의
export default function (sequelize: Sequelize) {
  return sequelize.define<Model<UserFriendsAttributes>>(
    "user_friends",
    {
        friend_id: {
    ...
    {
      tableName: "user_friends",  // 실제 DB에서 사용될 테이블 이름
