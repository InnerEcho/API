...
type MissionType = 'instant' | 'action' | 'ar_optional' | 'habit';
type MissionArAction = 'PET' | 'JUMP' | 'WATER' | 'SUNLIGHT';
...
export interface MissionAttributes {
  ...
  expReward: number;
  arBonusExp: number;
  requiresArAction: MissionArAction | null;
  cooldownSec: number;
  isActive: number;
  createdAt: Date;
  updatedAt: Date;
...
export type MissionCreationAttributes = Optional<
  MissionAttributes,
  | 'missionId'
  | 'desc'
  | 'expReward'
...
export default function missionModel(sequelize: Sequelize) {
  const Mission = sequelize.define<Model<MissionAttributes, MissionCreationAttributes>>('mission', {
    missionId: {
      field: 'mission_id',
